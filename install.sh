...
install_system_deps() {
    ...
    if command -v apt-get &>/dev/null; then
        ...
        apt-get install -y -qq python3 python3-pip python3-venv git curl unzip postgresql postgresql-contrib >> "$LOG_FILE" 2>&1
    else
        yum install -y python3 python3-pip git curl unzip postgresql-server postgresql-contrib >> "$LOG_FILE" 2>&1
    fi
...
collect_config() {
    ...
    # 6.5) Database Choice
    echo ""
    echo -e "${CYAN}7) Database Engine:${NC}"
    echo "   [1] SQLite — default (Easy setup)"
    echo "   [2] PostgreSQL (Better performance for large servers)"
