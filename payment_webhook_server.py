...
def approve_payment(payment_id: int) -> bool:
    """Atomically moves a payment from 'pending' to 'approved'. The
    previous version here did a SELECT to check status, THEN a separate
    UPDATE with no status condition — despite the docstring's claim, that
    is NOT race-safe: two concurrent approvals (e.g. this webhook and a
    human admin clicking approve at the same instant) could both pass the
    SELECT check before either UPDATE runs, double-crediting the user's
    balance. The WHERE clause on the UPDATE itself is what makes this
    actually atomic — only one caller's UPDATE can ever match the row."""
    with get_conn() as conn:
        cur = conn.execute(
